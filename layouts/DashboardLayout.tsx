...
const getCurrentUser = (): User => {
  const stored = localStorage.getItem("giva_user");
    ...
    if (role === "JUDGE") {
      return [
        ...
        },
        {
          label: "Events",
          icon: <Calendar size={20} />,
          href: "/dashboard/judge/events",
          id: "events",
        },
        {
          label: "Certificates",
          icon: <Award size={20} />,
          href: "/dashboard/judge/certificates",
          id: "certificates",
        },
        {
          label: "Profile",
          icon: <UserIcon size={20} />,
          href: "/dashboard/judge/profile",
          id: "profile",
        },
      ];
    }
          ...
          {navItems.map((item) => {
            ...
            const isActive =
              ...
              location.pathname.startsWith(
                item.href.replace("/list", "").replace("/manage", ""),
              );
            ...
            return (
              ...
              >
                ...
                <span
                  className={`shrink-0 ${isActive ? "text-primary-400" : "text-slate-500 group-hover:text-slate-300"}`}
                >
                  {item.icon}
                </span>
                {!collapsed && (
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full">
        ...
        <header className="lg:hidden h-16 bg-white border-b border-slate-100 flex items-center justify-between px-4 z-20 sticky top-0 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate("/dashboard?view=notifications")}
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 lg:p-8 custom-scrollbar bg-slate-50 pb-24 lg:pb-8">
          ...
          <div className="flex justify-around items-center h-16">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <div
                    className={`p-1 rounded-xl transition-all ${isActive ? "bg-primary-50 translate-y-[-2px]" : ""}`}
                  >
                    {item.icon}
                  </div>
                  <span className="text-[10px] font-bold truncate max-w-[56px] text-center leading-tight">
                    {item.label}
                  </span>
                </button>
              );
