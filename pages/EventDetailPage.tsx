...
import { 
  Calendar, MapPin, Share2, Clock, Globe, Users, 
  FileText, CheckCircle, ChevronDown, Download, Bookmark,
  Link as LinkIcon, Check, Trophy, Target,
  Layers, Lock, ThumbsUp, MessageCircle, Send, X,
  Linkedin, Twitter, User, Heart
} from 'lucide-react';
...
const getRichEventData = (id: string): RichEventData => {
  return {
    ...
    subThemes: [
      { title: "Smart Grid Adaptation", desc: "Hardware/Software for unstable energy grids.", icon: <Globe size={20} /> },
      { title: "Flood Defense Systems", desc: "Civil engineering and IoT monitoring.", icon: <Target size={20} /> },
      { title: "Emergency Response AI", desc: "Predictive modeling for disaster relief.", icon: <Users size={20} /> }
    ],
    judgingCriteria: [
      { category: "Innovation & Originality", weight: 30, desc: "Is the solution novel and distinct from existing approaches?" },
      { category: "Technical Feasibility", weight: 25, desc: "Can it be built and scaled with current technology?" },
      { category: "Impact Potential", weight: 25, desc: "Does it significantly address the SDG targets?" },
      { category: "Business Model", weight: 10, desc: "Is there a viable path to market or sustainability?" },
      { category: "Presentation", weight: 10, desc: "Clarity of the pitch and prototype demo." }
    ],
    requirements: [
    ...
    rules: [
      { title: "Team Composition", content: "Teams must consist of 1 to 5 members. Interdisciplinary teams are encouraged." },
      { title: "Originality", content: "All work must be original. Pre-existing projects must disclose previous development." },
      { title: "IP Rights", content: "Teams retain 100% ownership of their Intellectual Property." }
    ],
    judges: [
      { 
        id: "j1",
    ...
    qa: [
      {
        ...
        upvotes: 12,
        replies: [
          { user: "Event Admin", text: "For the prelim round, a high-fidelity simulation is acceptable. Finals require a physical MVP.", time: "1 day ago" }
        ]
      },
      {
        id: 2,
...
const EventDetailPage: React.FC = () => {
  ...
  const richData = getRichEventData(id || '');
  ...
  const QuickInfoCard = ({ icon, label, value, subValue }: { icon: React.ReactNode, label: string, value: string | number, subValue?: string }) => (
    <div className="flex flex-col items-center justify-center p-4 bg-white rounded-xl border border-slate-100 shadow-sm text-center h-full hover:border-primary-200 transition-colors">
      ...
      {subValue && <div className="text-xs text-slate-400 font-medium mb-1">{subValue}</div>}
    ...
    <div className="bg-slate-50 min-h-screen pb-24 lg:pb-0 font-sans text-slate-900">
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-20 max-w-7xl relative z-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8 lg:items-end">
            ...
            <div className="hidden lg:flex flex-col gap-4 w-72 shrink-0 pb-2">
              <Button size="lg" className="w-full shadow-xl shadow-primary-900/50" onClick={() => document.getElementById('register-section')?.scrollIntoView({behavior: 'smooth'})}>
                Register Now
              </Button>
      ...
      <div className="container mx-auto px-6 md:px-12 lg:px-20 max-w-7xl -mt-12 relative z-20">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0 space-y-12 pb-24">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <QuickInfoCard 
                icon={<Users />} 
                label="Registered" 
                value={isRegistered ? richData.stats.registered + 1 : richData.stats.registered} 
              ...
              <QuickInfoCard 
                icon={<Bookmark />} 
                label="Interested" 
                value={isSaved ? richData.stats.saved + 1 : richData.stats.saved} 
                subValue="Saves"
              />
              {/* Eligibility */}
              <QuickInfoCard icon={<CheckCircle />} label="Eligibility" value={richData.eligibilityLevel} />
              
              <QuickInfoCard icon={<Users />} label="Team Size" value={richData.teamSize} />
              <QuickInfoCard icon={<Target />} label="Entry Fee" value={richData.fee} />
              <QuickInfoCard icon={<Globe />} label="Access" value={event.format} />
            </div>
            ...
            <section className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-10">
              ...
              {/* Theme */}
              <div>
                <h3 className="text-xl font-bold text-slate-900 mb-4">Core Theme</h3>
                <div className="bg-primary-50 p-6 rounded-2xl border border-primary-100">
                  <h4 className="text-2xl font-bold text-primary-700 mb-2">"{richData.theme}"</h4>
                  <p className="text-primary-800/70 text-sm">
                    All submissions must align with this central theme to be considered for evaluation.
                  </p>
                </div>
              </div>
              ...
              {/* Tracks */}
              <div>
                <h3 className="text-xl font-bold text-slate-900 mb-6">Sub-Themes & Tracks</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {richData.subThemes.map((track, i) => (
                    <div key={i} className="bg-slate-50 p-5 rounded-xl border border-slate-100 hover:border-primary-300 transition-colors group">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-900 text-sm mb-1">{track.title}</h4>
            ...
            <section>
              <SectionHeading title="Judging & Grading Criteria" subtitle="How your submission will be evaluated." />
                   ...
                   {richData.judgingCriteria.map((crit, i) => (
                     <div key={i}>
                       <div className="flex justify-between items-end mb-2">
                         <div>
                           <div className="font-bold text-slate-900">{crit.category}</div>
                           <div className="text-xs text-slate-500">{crit.desc}</div>
                         </div>
                         <div className="font-bold text-primary-600 text-lg">{crit.weight}%</div>
                       </div>
            ...
            {/* 5. TIMELINE */}
            <section>
              <SectionHeading title="Event Timeline" subtitle="Key dates and milestones you need to know." />
              <div className="bg-white rounded-3xl border border-slate-200 p-8">
              ...
              <section className="bg-gradient-to-br from-slate-900 to-primary-900 rounded-3xl p-8 md:p-12 text-white relative overflow-hidden">
                 ...
                 <div className="relative z-10 text-center">
                   ...
                   <p className="text-primary-200 mb-10 max-w-lg mx-auto">
                     Top performing teams will receive non-dilutive grant funding, incubation support, and global media coverage.
                   </p>
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
                     <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                       ...
                       <div className="text-xs text-primary-200">Grant + Incubation</div>
                     </div>
                     <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                       <div className="text-slate-300 font-bold text-xl mb-1">2nd Place</div>
                       <div className="text-2xl font-bold mb-2">$15,000</div>
                       <div className="text-xs text-primary-200">Grant + Mentorship</div>
                     </div>
                     <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-white/10">
                       <div className="text-amber-600 font-bold text-xl mb-1">3rd Place</div>
                       <div className="text-2xl font-bold mb-2">$10,000</div>
                       <div className="text-xs text-primary-200">Grant + Perks</div>
                     </div>
                   </div>
                 </div>
              </section>
            ...
            <section>
              ...
              <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex-1 bg-white p-8 rounded-2xl border border-slate-200 min-h-[200px]">
                  <h4 className="text-xl font-bold text-slate-900 mb-4">{richData.rules[activeRuleTab].title}</h4>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {richData.resources.map((res, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-xl hover:border-primary-400 hover:shadow-sm transition-all cursor-pointer group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 text-sm group-hover:text-primary-700">{res.name}</div>
                        <div className="text-xs text-slate-500">{res.type} • {res.size}</div>
                      </div>
                    </div>
            ...
            <section id="qa">
              <SectionHeading title="Public Q&A" subtitle="Ask questions and get answers from the organizers and community." />
                ...
                <div className="mb-8">
                  {isLoggedIn ? (
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex justify-end mt-2">
                           <Button size="sm" className="gap-2" onClick={() => setNewQuestion('')} disabled={!newQuestion.trim()}>
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 text-center">
                      ...
                      <h4 className="font-bold text-slate-900 mb-1">Join the conversation</h4>
                      <p className="text-sm text-slate-500 mb-4">Please log in to ask questions or reply to others.</p>
                      ...
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                             <span className="font-bold text-slate-900 text-sm">{q.user}</span>
                             <span className="text-xs text-slate-400">• {q.time}</span>
                          </div>
                          <p className="text-slate-700 text-sm mb-3">{q.text}</p>
                                ...
                                <div key={i} className="flex gap-3">
                                   ...
                                   <div>
                                     <div className="flex items-center gap-2 mb-0.5">
                                       <span className="font-bold text-slate-800 text-xs">{reply.user}</span>
                                       <span className="px-1.5 py-0.5 bg-primary-600 text-white text-[9px] font-bold uppercase rounded">Organizer</span>
            ...
            <section className="bg-primary-50 rounded-3xl p-8 md:p-12 text-center border border-primary-100">
              <h2 className="text-3xl font-bold text-slate-900 mb-6">Ready to make your mark?</h2>
              <p className="text-slate-600 mb-8 max-w-xl mx-auto">
                Join hundreds of innovators from around the world. Registration closes on {richData.deadline}.
              </p>
              <div className="flex justify-center gap-4">
                <Button size="lg" onClick={() => setIsRegistered(true)}>
                  Register Now
                </Button>
                <Button variant="white" size="lg" className="border-primary-200 text-primary-700">
                  Contact Organizers
                </Button>
              </div>
            </section>
          ...
          <div className="hidden lg:block w-[360px] shrink-0" id="register-section">
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6">
                <div className="mb-6">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    {isRegistered && <span className="text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded">Registered</span>}
                  </div>
                  <div className="flex items-center gap-2 mb-4">
                    <div className={`w-3 h-3 rounded-full ${event.status === 'Open' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`}></div>
                    <span className="font-bold text-xl text-slate-900">{event.status === 'Open' ? 'Registration Open' : 'Registration Closed'}</span>
                  </div>
                  <p className="text-sm text-slate-500">
                    Closes on <span className="font-bold text-slate-700">{richData.deadline}</span>
                  </p>
                </div>
                ...
                {isRegistered ? (
                  <div className="space-y-3">
                    <div className="bg-emerald-50 border border-emerald-100 rounded-lg p-3 flex items-start gap-3">
                      ...
                      <p className="text-xs text-emerald-800 font-medium">You're all set! Check your dashboard for next steps.</p>
                    </div>
                    <Button fullWidth variant="outline" size="sm">Go to Dashboard</Button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <Button fullWidth size="lg" disabled={event.status === 'Closed'} onClick={() => setIsRegistered(true)}>
                      {event.status === 'Closed' ? 'Closed' : 'Register for Event'}
                    </Button>
              ...
              <div className="bg-slate-900 rounded-2xl overflow-hidden shadow-lg relative group aspect-[3/4]">
                 <img src={richData.poster} alt="Event Poster" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-500" />
                 <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-6">
                    <span className="text-xs font-bold text-white/60 uppercase mb-1">Official Poster</span>
                    <h4 className="font-bold text-white leading-tight">Download & Share</h4>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6">
                <h4 className="font-bold text-slate-900 mb-4">Featured Judges</h4>
                <div className="space-y-3">
                  {richData.judges.map((judge) => (
                    ...
                    >
                      <img src={judge.img} alt={judge.name} className="w-10 h-10 rounded-full object-cover border border-slate-100" />
                      <div>
                        <div className="font-bold text-sm text-slate-900 group-hover:text-primary-600 transition-colors">{judge.name}</div>
                        <div className="text-xs text-slate-500">{judge.role}</div>
                      </div>
                      <ChevronDown className="ml-auto text-slate-300 -rotate-90" size={16} />
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 z-40 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] safe-area-bottom">
         <div className="flex gap-3">
           <div className="flex-1">
              <Button fullWidth onClick={() => setIsRegistered(true)} disabled={isRegistered || event.status === 'Closed'}>
                {isRegistered ? 'Manage' : (event.status === 'Closed' ? 'Closed' : 'Register Now')}
              </Button>
           </div>
           <button 
            onClick={handleCopyLink}
      ...
      {selectedJudge && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setSelectedJudge(null)}></div>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 animate-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="px-8 pb-8 -mt-12">
               <img src={selectedJudge.img} alt={selectedJudge.name} className="w-24 h-24 rounded-full border-4 border-white shadow-md mb-4" />
               <h3 className="text-2xl font-bold text-slate-900">{selectedJudge.name}</h3>
               <p className="text-slate-500 font-medium mb-1">{selectedJudge.role} at {selectedJudge.org}</p>
               
               <div className="flex gap-3 mt-4 mb-6">
                 {selectedJudge.socials.linkedin && (
                   <a href={selectedJudge.socials.linkedin} className="p-2 bg-slate-100 text-slate-600 rounded-full hover:bg-blue-50 hover:text-blue-600 transition-colors">
                     <Linkedin size={18} />
                   </a>
                 )}
                  {selectedJudge.socials.twitter && (
                   <a href={selectedJudge.socials.twitter} className="p-2 bg-slate-100 text-slate-600 rounded-full hover:bg-sky-50 hover:text-sky-500 transition-colors">
                     <Twitter size={18} />
                   </a>
                 )}
               </div>
               ...
               <div className="space-y-4">
                 <div>
                   <h4 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-2">Bio</h4>
                   <p className="text-slate-600 text-sm leading-relaxed">{selectedJudge.bio}</p>
                 </div>
                 <div>
                   <h4 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-2">Expertise</h4>
                   <div className="flex flex-wrap gap-2">
                     {selectedJudge.expertise.map(exp => (
                       <span key={exp} className="px-2.5 py-1 bg-primary-50 text-primary-700 text-xs font-bold rounded-full border border-primary-100">
                         {exp}
                       </span>
                     ))}
                   </div>
               ...
               <div className="mt-8 pt-6 border-t border-slate-100 text-center">
                 <button onClick={() => setSelectedJudge(null)} className="text-sm font-bold text-primary-600 hover:underline">Close Profile</button>
               </div>
            </div>
          </div>
        </div>
