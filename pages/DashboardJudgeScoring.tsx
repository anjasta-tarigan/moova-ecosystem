...
import { 
  ChevronLeft, FileText, Youtube, Users, MessageSquare, 
  Save, CheckCircle, AlertCircle, Info, Maximize2, Download,
  ExternalLink, Lock
} from 'lucide-react';
...
const SUBMISSION = {
  ...
  team: 'AgriVision',
  track: 'AI/ML',
  abstract: 'We propose a lightweight CNN model deployable on edge devices (Raspberry Pi) to detect early onset of cassava mosaic disease in rural African farms. Our solution works offline and provides actionable treatment advice.',
  problem: 'Cassava mosaic disease causes $2B+ in annual losses. Farmers lack access to experts for early diagnosis.',
  solution: 'A mobile app connected to a specialized camera rig that scans leaves and uses our custom model to identify pathogens with 94% accuracy.',
...
const RUBRIC = [
  { id: 'c1', label: 'Innovation & Novelty', weight: 30, max: 10, desc: 'Is the solution unique compared to existing alternatives?' },
  { id: 'c2', label: 'Technical Feasibility', weight: 30, max: 10, desc: 'Is the implementation sound and technically viable?' },
  { id: 'c3', label: 'Impact Potential', weight: 25, max: 10, desc: 'Does it significantly address the target SDG?' },
  { id: 'c4', label: 'Presentation Quality', weight: 15, max: 10, desc: 'Clarity of the pitch, video, and documentation.' },
];
...
const DashboardJudgeScoring: React.FC = () => {
  ...
  // --- State ---
  const [activeTab, setActiveTab] = useState<'overview' | 'files' | 'video' | 'team'>('overview');
      ...
      <div className="flex-1 flex flex-col md:w-3/5 lg:w-2/3 h-full overflow-hidden border-r border-slate-200 bg-white">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white z-10 shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/dashboard')} className="p-2 hover:bg-slate-50 rounded-lg text-slate-500 transition-colors">
              <ChevronLeft size={20} />
            </button>
            <div>
              <h1 className="text-xl font-bold text-slate-900 line-clamp-1">{SUBMISSION.title}</h1>
              <p className="text-xs text-slate-500">by {SUBMISSION.team} • {SUBMISSION.track}</p>
            </div>
          </div>
            ...
            {['overview', 'files', 'video', 'team'].map(tab => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-colors ${
                  activeTab === tab ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'
                }`}
              >
                {tab}
              </button>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-8 max-w-3xl mx-auto animate-in fade-in duration-300">
              <div>
                <h3 className="text-sm font-bold uppercase text-slate-400 mb-2 tracking-wider">Abstract</h3>
                <p className="text-lg text-slate-800 leading-relaxed font-medium">{SUBMISSION.abstract}</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-red-50 p-6 rounded-2xl border border-red-100">
                  <h3 className="text-sm font-bold uppercase text-red-800 mb-2 tracking-wider">The Problem</h3>
                  <p className="text-slate-700 text-sm leading-relaxed">{SUBMISSION.problem}</p>
                </div>
                <div className="bg-emerald-50 p-6 rounded-2xl border border-emerald-100">
                  <h3 className="text-sm font-bold uppercase text-emerald-800 mb-2 tracking-wider">The Solution</h3>
                  <p className="text-slate-700 text-sm leading-relaxed">{SUBMISSION.solution}</p>
                </div>
              </div>
              <div>
                <h3 className="text-sm font-bold uppercase text-slate-400 mb-3 tracking-wider">Tech Stack</h3>
              ...
              <div className="flex gap-4 pt-6 border-t border-slate-100">
                <a href={SUBMISSION.repo} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-primary-600 font-bold text-sm hover:underline">
          ...
          {activeTab === 'files' && (
            <div className="space-y-4 animate-in fade-in duration-300 max-w-2xl mx-auto">
              <h3 className="text-lg font-bold text-slate-900 mb-4">Attached Documents</h3>
              {SUBMISSION.files.map((file, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-200 rounded-xl hover:border-primary-300 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-500 shadow-sm">
                      <FileText size={20} />
                    </div>
                    <div>
                      <div className="font-bold text-slate-900 text-sm group-hover:text-primary-700">{file.name}</div>
          ...
          {activeTab === 'video' && (
            <div className="animate-in fade-in duration-300 max-w-3xl mx-auto">
              ...
              <p className="mt-4 text-sm text-slate-500 text-center">Video Demo Submission</p>
            </div>
          )}
              ...
              {SUBMISSION.members.map((m, i) => (
                <div key={i} className="flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-xl shadow-sm">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-slate-500 uppercase tracking-wide">{m.role}</div>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="w-full md:w-2/5 lg:w-1/3 bg-slate-50 flex flex-col h-full border-l border-slate-200 shadow-xl z-20">
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-200 flex justify-between items-center shrink-0">
          ...
          <div className="text-right">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Total Score</span>
            <span className={`text-2xl font-black ${totalScore > 0 ? 'text-primary-600' : 'text-slate-300'}`}>
              {totalScore} <span className="text-sm text-slate-400 font-medium">/ 100</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scrollbar relative">
          {isLocked && (
            <div className="absolute inset-0 bg-white/50 backdrop-blur-[2px] z-10 flex flex-col items-center justify-center p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-slate-900">Grading Locked</h3>
              <p className="text-slate-600 text-sm mt-2 mb-6">
                You have submitted the final score for this project. To make changes, please contact the event administrator.
              </p>
          ...
          <div className="space-y-8 pb-8">
            {RUBRIC.map((criterion) => (
              <div key={criterion.id} className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <label className="font-bold text-slate-900 text-sm">{criterion.label}</label>
                  <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-1 rounded">Weight: {criterion.weight}%</span>
                </div>
                <p className="text-xs text-slate-500 mb-4 leading-snug">{criterion.desc}</p>
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
              ...
              <textarea 
                ...
                onChange={(e) => !isLocked && setGeneralFeedback(e.target.value)}
                disabled={isLocked}
