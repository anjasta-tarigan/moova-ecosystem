...
import { 
  ArrowLeft, Lock, Mail, Info, AlertCircle 
} from 'lucide-react';
...
const DEMO_ACCOUNTS = [
  ...
  { 
    email: 'reviewer@moova.test', 
    password: 'reviewer123', 
    role: 'reviewer', 
    name: 'Reviewer', 
    lastName: 'One',
    org: 'Scientific Committee', 
    redirect: '/dashboard' 
  }
];
...
const AuthPage: React.FC = () => {
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const matchedAccount = DEMO_ACCOUNTS.find(
        acc => acc.email === emailInput && acc.password === passInput
      );
      ...
      if (matchedAccount) {
        ...
        const sessionUser = {
          id: 'u-' + Date.now(),
          firstName: matchedAccount.name,
          lastName: matchedAccount.lastName,
          email: matchedAccount.email,
          role: matchedAccount.role as UserRole,
          organization: matchedAccount.org
        };

        localStorage.setItem('moova_user', JSON.stringify(sessionUser));
        setLoading(false);
  ...
  const SocialButton: React.FC<{ icon: string; label: string }> = ({ icon, label }) => (
    <button type="button" className="flex items-center justify-center gap-3 w-full py-3 px-4 border border-primary-200 rounded-xl text-primary-900 font-medium hover:bg-primary-50 hover:border-primary-300 hover:shadow-sm transition-all text-sm group">
      <img src={icon} alt="" className="w-5 h-5 opacity-80 group-hover:opacity-100 transition-opacity" />
      <span>{label}</span>
    </button>
  );
    ...
    <div className="min-h-screen flex w-full bg-white font-sans text-primary-900 selection:bg-secondary-100 selection:text-secondary-900">
      ...
      <div className="hidden lg:flex lg:w-5/12 relative bg-primary-900 flex-col justify-between p-12 text-white overflow-hidden">
         ...
         <div className="relative z-10 max-w-md animate-in slide-in-from-bottom-8 duration-1000 delay-150 fill-mode-both">
            <h2 className="text-4xl xl:text-5xl font-bold leading-tight mb-6">
              Accelerating the <span className="text-transparent bg-clip-text bg-brand-gradient">velocity of science.</span>
            </h2>
            <p className="text-primary-100/70 text-lg leading-relaxed">
              Join the ecosystem where ideas evolve into impact through structured collaboration and global connectivity.
            </p>
         </div>

         <div className="relative z-10">
            <p className="text-xs font-bold uppercase tracking-widest text-primary-300 mb-4">Supported By</p>
      ...
      <div className="flex-1 flex flex-col relative bg-white h-screen overflow-y-auto">
        ...
        <div className="flex justify-between items-center p-6 lg:p-12 absolute top-0 w-full z-20 pointer-events-none">
           <Link to="/" className="pointer-events-auto flex items-center gap-2 text-sm font-medium text-primary-500 hover:text-primary-900 transition-colors group">
           ...
           <div className="lg:hidden pointer-events-auto flex items-center gap-2">
              <div className="w-8 h-8 bg-brand-gradient rounded-lg flex items-center justify-center text-white font-bold text-sm">M</div>
           </div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 lg:px-24 w-full max-w-2xl mx-auto py-24">
          ...
          <div className="mb-8 animate-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center gap-2 text-sm text-primary-600 font-medium bg-primary-50 p-3 rounded-lg border border-primary-100 w-fit">
              ...
              <Link to={isLogin ? "/join" : "/login"} className="text-secondary-500 hover:text-secondary-600 font-bold hover:underline transition-colors">
          ...
          {!isLogin && (
            <div className="mb-8 animate-in slide-in-from-bottom-4 duration-700 delay-100">
              <div className="flex items-center gap-2 mb-2">
                 <span className="text-xs font-bold uppercase text-primary-600 tracking-wider">Step {step} of {totalSteps}</span>
                 <span className="text-xs font-bold uppercase text-primary-400 tracking-wider">• {step === 1 ? 'Identity' : step === 2 ? 'Background' : 'Role'}</span>
              </div>
            ...
            {isLogin ? (
              <form onSubmit={handleLoginSubmit} className="space-y-6">
                ...
                <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs text-slate-500">
                  ...
                  user@moova.test / user123 (Participant)<br/>
                ...
                {error && (
                 ...
                 <div className="space-y-1">
                  <label className="flex items-center text-xs font-bold uppercase text-primary-500 mb-1.5 ml-1">Email Address</label>
                  <div className="relative group">
                    <Mail className="absolute left-4 top-3.5 text-primary-400 group-focus-within:text-primary-600 transition-colors" size={18} />
                    <input 
                      required 
                      type="email" 
                      name="email"
                ...
                <div className="space-y-1">
                  <div className="flex justify-between items-center mb-1.5 ml-1">
                    <label className="text-xs font-bold uppercase text-primary-500">Password</label>
                    <a href="#" className="text-xs font-bold text-secondary-500 hover:text-secondary-600 hover:underline">Forgot password?</a>
                  </div>
                  <div className="relative group">
                    <Lock className="absolute left-4 top-3.5 text-primary-400 group-focus-within:text-primary-600 transition-colors" size={18} />
                    <input 
                      required 
                      type="password" 
                      name="password"
                ...
                <div className="relative my-8">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white px-2 text-primary-400 font-bold tracking-wider">Or continue with</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                   <SocialButton icon="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg" label="Google" />
                   <SocialButton icon="https://upload.wikimedia.org/wikipedia/commons/4/44/Microsoft_logo.svg" label="Microsoft" />
                </div>
              </form>
            ) : (
               // Sign Up Flow
               <form onSubmit={handleNext} className="space-y-6">
                {step === 1 && (
                  <div className="space-y-5 animate-in fade-in slide-in-from-right-8 duration-300">
                    <div className="grid grid-cols-2 gap-5">
                      <div className="space-y-1">
                        <label className="text-xs font-bold uppercase text-primary-500 ml-1">First Name</label>
                        <input required name="firstName" value={formData.firstName} onChange={handleInputChange} type="text" className="w-full px-4 py-3 bg-primary-50/50 border border-primary-100 rounded-xl focus:ring-2 focus:ring-primary-100 focus:border-primary-500 outline-none transition-all text-sm font-medium" placeholder="Jane" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-bold uppercase text-primary-500 ml-1">Last Name</label>
                        <input required name="lastName" value={formData.lastName} onChange={handleInputChange} type="text" className="w-full px-4 py-3 bg-primary-50/50 border border-primary-100 rounded-xl focus:ring-2 focus:ring-primary-100 focus:border-primary-500 outline-none transition-all text-sm font-medium" placeholder="Doe" />
                      </div>
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs font-bold uppercase text-primary-500 ml-1">Email</label>
                        <input required name="email" value={formData.email} onChange={handleInputChange} type="email" className="w-full px-4 py-3 bg-primary-50/50 border border-primary-100 rounded-xl focus:ring-2 focus:ring-primary-100 focus:border-primary-500 outline-none transition-all text-sm font-medium" placeholder="email@example.com" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs font-bold uppercase text-primary-500 ml-1">Password</label>
                        <input required name="password" value={formData.password} onChange={handleInputChange} type="password" className="w-full px-4 py-3 bg-primary-50/50 border border-primary-100 rounded-xl focus:ring-2 focus:ring-primary-100 focus:border-primary-500 outline-none transition-all text-sm font-medium" placeholder="Min. 8 chars" />
                    </div>
                    
                    <div className="pt-6 mt-6 border-t border-primary-100">
                      <p className="text-xs text-center text-primary-400 mb-4 font-medium uppercase tracking-wide">Or register via SSO</p>
                      <div className="grid grid-cols-2 gap-4">
                         <SocialButton icon="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg" label="Google" />
                         <SocialButton icon="https://upload.wikimedia.org/wikipedia/commons/4/44/Microsoft_logo.svg" label="Microsoft" />
                      </div>
                ...
                {step === 2 && (
                   <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                     <div className="p-8 bg-primary-50/50 rounded-xl border border-primary-200 text-center border-dashed">
                       ...
                       <h3 className="text-primary-900 font-bold text-sm mb-1">Academic Background</h3>
                       <p className="text-sm text-primary-500">
                         This section would contain fields for Institution, Education Level, and Field of Interest.
                       </p>
                     </div>
                   </div>
                )}
                ...
                {step === 3 && (
                   <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                     <div className="p-8 bg-primary-50/50 rounded-xl border border-primary-200 text-center border-dashed">
                       ...
                       <h3 className="text-primary-900 font-bold text-sm mb-1">Role Selection</h3>
                       <p className="text-sm text-primary-500">
                         This section would confirm your primary role (Researcher, Founder, etc.) and collect legal consents.
                       </p>
                     </div>
                   </div>
                )}
                ...
                <div className="flex gap-4 mt-8">
                   ...
                   <Button type="submit" fullWidth size="lg" disabled={loading} className="flex-1 shadow-lg shadow-primary-500/20 hover:shadow-primary-500/40">
