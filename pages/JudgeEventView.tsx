...
import { ChevronLeft, Filter, Search, CheckCircle, Clock, AlertCircle, BarChart, ChevronRight } from 'lucide-react';
...
const JudgeEventView: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const data = await judgeService.getCategorySubmissions(categoryId, activeStage);
      setSubmissions(data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const filteredSubmissions = submissions.filter(sub => {
    const matchesStatus = filterStatus === 'all' 
      ? true 
      : filterStatus === 'pending' ? (sub.scoringStatus !== 'submitted') 
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'submitted':
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 border border-emerald-200"><CheckCircle size={12}/> Scored</span>;
      case 'draft':
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200"><Clock size={12}/> Draft</span>;
      default:
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200"><AlertCircle size={12}/> Pending</span>;
    }
  };
  ...
  const progress = submissions.length > 0 
    ? Math.round((submissions.filter(s => s.scoringStatus === 'submitted').length / submissions.length) * 100) 
    : 0;
    ...
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in duration-500 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold text-slate-900">Evaluation Workspace</h1>
          <p className="text-slate-500 text-sm">Category ID: <span className="font-mono text-slate-700">{categoryId}</span></p>
        </div>
        ...
        <div className="bg-white px-6 py-3 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-bold text-slate-500 uppercase">Stage Progress</div>
            <div className="text-lg font-bold text-slate-900 leading-none">{progress}% <span className="text-xs font-normal text-slate-400">Completed</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[600px]">
          ...
          {stages.map((stage, idx) => (
            ...
            >
              <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs border ${
                activeStage === stage.id ? 'bg-brand-blue text-white border-brand-blue' : 'bg-white border-slate-300'
              }`}>
                {idx + 1}
              </span>
              {stage.label}
        ...
        <div className="flex-1 bg-slate-50 p-6 overflow-y-auto">
          {loading ? (
            <div className="flex items-center justify-center h-64 text-slate-500">Loading submissions...</div>
            ...
            <div className="space-y-3">
              {filteredSubmissions.map((sub) => (
                <div 
                  key={sub.id} 
                  onClick={() => navigate(`/dashboard/judge/events/${eventId}/round/${activeStage}/submission/${sub.id}`)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-center md:text-left">
                    ...
                    <div className="text-sm text-slate-500 flex flex-wrap gap-x-4 justify-center md:justify-start">
                      ...
                      <span className="text-slate-300">|</span>
                      <span className="font-mono text-xs pt-0.5 opacity-70">ID: {sub.id}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-6 shrink-0 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-slate-100 pt-3 md:pt-0">
                    {sub.scoringStatus === 'submitted' ? (
                      <div className="text-right">
                        <span className="block text-2xl font-black text-slate-900">{sub.totalScore}</span>
        ...
        <div className="bg-white border-t border-slate-200 px-6 py-3 text-xs text-slate-500 flex justify-between">
          ...
          <span>Evaluation Criteria: <strong className="uppercase">{activeStage}</strong></span>
        </div>
      </div>
    </div>
  );
