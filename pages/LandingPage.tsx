...
import { 
  ArrowRight, Globe, Users, Zap, 
  TrendingUp, Rocket, Microscope,
  ChevronRight, CheckCircle, X, ExternalLink,
...
const ALL_SDGS = [
  { id: 1, title: "No Poverty", color: "bg-slate-900", icon: "🌱", desc: "End poverty in all its forms everywhere.", relevance: "Creating economic opportunity through entrepreneurship.", example: "Micro-finance blockchain solutions." },
  { id: 2, title: "Zero Hunger", color: "bg-slate-900", icon: "🌾", desc: "End hunger, achieve food security and sustainable agriculture.", relevance: "AgriTech for arid climates and supply chain optimization.", example: "Smart irrigation drone pilots." },
  { id: 3, title: "Good Health", color: "bg-slate-900", icon: "❤️", desc: "Ensure healthy lives and promote well-being for all.", relevance: "MedTech diagnostics and accessible healthcare platforms.", example: "AI-driven disease detection apps." },
  { id: 4, title: "Quality Education", color: "bg-slate-900", icon: "🎓", desc: "Ensure inclusive and equitable quality education.", relevance: "EdTech platforms and democratizing access to science.", example: "Virtual STEM labs for remote schools." },
  { id: 5, title: "Gender Equality", color: "bg-slate-900", icon: "⚖️", desc: "Achieve gender equality and empower all women and girls.", relevance: "Supporting female founders and bias-free AI.", example: "Women in Deep Tech Fellowship." },
  ...
  { id: 13, title: "Climate Action", color: "bg-slate-900", icon: "🌍", desc: "Take urgent action to combat climate change.", relevance: "Carbon capture, climate modeling, and resilience tech.", example: "Carbon credit verification platforms." },
  { id: 14, title: "Life Below Water", color: "bg-slate-900", icon: "🐠", desc: "Conserve and sustainably use the oceans.", relevance: "Blue economy, ocean cleaning, and aquaculture.", example: "Ocean micro-plastic sensors." },
  { id: 15, title: "Life on Land", color: "bg-slate-900", icon: "🌳", desc: "Protect, restore and promote sustainable use of terrestrial ecosystems.", relevance: "Biodiversity monitoring and reforestation tech.", example: "Drone-based reforestation." },
  { id: 16, title: "Peace & Justice", color: "bg-slate-900", icon: "🕊️", desc: "Promote peaceful and inclusive societies.", relevance: "Civic tech, transparency tools, and digital rights.", example: "Anti-corruption blockchain tools." },
  { id: 17, title: "Partnerships", color: "bg-slate-900", icon: "🔗", desc: "Strengthen the means of implementation.", relevance: "Connecting silos: Academia, Industry, and Government.", example: "The Global Science Summit." },
];

const STATS_TICKER = [
  { label: "Planned Pilots", value: "3" },
  { label: "Core Modules", value: "5" },
  { label: "SDGs Integrated", value: "17" },
  { label: "Launch Phase", value: "Alpha" },
];
...
const LandingPage: React.FC = () => {
  ...
  const [selectedSdg, setSelectedSdg] = useState<typeof ALL_SDGS[0] | null>(null);
  ...
  return (
    <div className="flex flex-col w-full bg-white overflow-hidden">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 min-h-[95vh] flex flex-col items-center justify-center overflow-hidden bg-white">
        ...
        {/* Cyan Glow - Top Left */}
        <div className="absolute top-[-20%] left-[-10%] w-[800px] h-[800px] rounded-full blur-[160px] opacity-20 pointer-events-none" style={{ background: '#00BBFF' }}></div>
        {/* Blue Glow - Bottom Right */}
        <div className="absolute bottom-[-10%] right-[-5%] w-[900px] h-[900px] rounded-full blur-[180px] opacity-10 pointer-events-none" style={{ background: '#0052CC' }}></div>
        {/* Subtle Crimson Pulse */}
        <div className="absolute top-[20%] right-[10%] w-[300px] h-[300px] rounded-full blur-[120px] opacity-5 pointer-events-none animate-pulse-slow" style={{ background: '#DC143C' }}></div>
        ...
        <div className="container mx-auto px-6 relative z-10 text-center flex flex-col items-center">
            ...
            {/* Buttons (Monochrome) */}
            <FadeIn delay={400} className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto">
               <Button size="lg" onClick={() => navigate('/join')} className="h-14 px-8 text-base w-full sm:w-auto font-bold rounded-full bg-slate-900 text-white hover:bg-black border border-transparent shadow-lg shadow-slate-200/50">
                 Create Free Account
               </Button>
               <Button variant="white" size="lg" onClick={() => navigate('/programs')} className="h-14 px-8 text-base border border-slate-200 text-slate-900 w-full sm:w-auto group font-bold rounded-full hover:bg-slate-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-16">
          {[
            ...
            <FadeIn key={i} delay={i * 150} className="h-full">
              <div className={`h-full bg-white p-8 rounded-3xl border border-slate-100 ${item.glow} hover:shadow-xl hover:border-slate-300 transition-all duration-500 group cursor-default relative overflow-hidden`}>
                
                <span className="text-4xl font-black text-slate-100 mb-6 block group-hover:text-slate-200 transition-colors">0{i + 1}</span>
                
                <div className={`w-14 h-14 bg-slate-50 rounded-xl flex items-center justify-center text-slate-900 mb-6 relative z-10 group-hover:bg-slate-900 group-hover:text-white transition-colors duration-300`}>
                  {item.icon}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {STAKEHOLDERS.map((item, i) => (
              <FadeIn key={i} delay={i * 100}>
                ...
                >
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6 relative z-10 bg-slate-100 text-slate-900`}>
                    {item.icon}
                  </div>
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-slate-300 text-sm mb-6">
                    Don't fit a label? If you are building the future, you belong here.
                  </p>
          ...
          <div className="max-w-4xl mb-12">
            <FadeIn>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10 mb-6 backdrop-blur-md">
                ...
                <span className="text-xs font-bold text-white uppercase tracking-widest">Global Agenda</span>
              </div>
            ...
            {ALL_SDGS.map((sdg, i) => (
              <FadeIn key={i} delay={Math.min(i * 50, 1000)}>
                ...
                >
                   ...
                   <div className={`absolute top-0 left-0 w-full h-1 bg-white opacity-0 group-hover:opacity-100 transition-opacity`}></div>

                   <div className="flex justify-between items-start">
                     <span className="text-xl font-bold text-white/20 group-hover:text-white/60 transition-colors font-mono">
                       {sdg.id}
                     </span>
                     <span className="text-xl filter grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:scale-110">{sdg.icon}</span>
                   </div>
        ...
        <div className="container mx-auto px-6 md:px-12 lg:px-20 max-w-7xl text-center relative z-10">
          <FadeIn className="max-w-3xl mx-auto">
             ...
             <div className="flex flex-col sm:flex-row gap-4 justify-center">
               ...
               <Button variant="outline" size="lg" onClick={() => navigate('/events')} className="h-16 px-12 text-lg rounded-full bg-white border-slate-200 text-slate-800">
                 Browse Events
               </Button>
             </div>
             <div className="mt-12 flex items-center justify-center gap-2 text-sm text-slate-400 font-medium">
               ...
               <span className="ml-2">Join the waitlist for our inaugural cohort.</span>
             </div>
          </FadeIn>
        </div>
      </section>
      ...
      {selectedSdg && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setSelectedSdg(null)}></div>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col max-h-[90vh]">
             ...
             {/* Modal Header */}
             <div className={`h-40 bg-slate-900 relative flex flex-col justify-end p-8 text-white shrink-0`}>
                <button onClick={() => setSelectedSdg(null)} className="absolute top-4 right-4 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors">
                  <X size={20} />
                </button>
                <div className="absolute top-6 right-8 text-8xl opacity-10">{selectedSdg.icon}</div>
                <div className="relative z-10">
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80 mb-1 block">Goal {selectedSdg.id}</span>
             ...
             <div className="p-8 overflow-y-auto">
               <div className="mb-8">
                 <h4 className="text-sm font-bold uppercase text-slate-400 mb-2 tracking-wide">Mission</h4>
               ...
               <div className="mt-8 pt-6 border-t border-slate-100">
                 <Button fullWidth onClick={() => { navigate('/programs'); setSelectedSdg(null); }}>
                   Find Related Programs <ArrowRight size={16} className="ml-2" />
                 </Button>
               </div>
             </div>
          </div>
